...
$\mathcal {AS} ( \pi_F \cup \pi_m \cup \pi_{m(F)} \cup \pi_\sigma \cup \pi_{\hat \sigma} \cup \pi_{\hat E} \cup \pi_{m(X) = \hat X} ) \cong \\{ X, \hat E \mid X \in \sigma(F) \land \hat E \in \hat \sigma(\hat F) \land m(X) = \hat E \land \hat X = \hat E \\}$
